...
impl Config {
    fn from_buf_reader(f: &mut BufReader<&[u8]>) -> Self {
        let c = Self {
            dim: read::<i32>(f),
...
impl TransformerWeights {
    fn from_buf_reader(f: &mut BufReader<&[u8]>, c: &Config) -> Self {
...
type Score = f32;
fn read_tokenizer(tokenizer_file_bytes: Vec<u8>, vocab_size: usize) -> (Vec<(String, Score)>, u32) {

    let mut rdr = BufReader::new(&tokenizer_file_bytes[..]);
...
fn read<T: FromBytes>(rdr: &mut BufReader<&[u8]>) -> T {
    let mut buffer = [0u8; 4];
    rdr.read_exact(&mut buffer).expect("Error reading file");
    T::from_bytes(buffer)
}

fn read_vec<T: FromBytes>(rdr: &mut BufReader<&[u8]>, size: i32) -> Vec<T> {
...
fn main() {
    ...
    */
    let ckpt_file_bytes: Vec<u8> = env::read();
    let tokenizer_file_bytes: Vec<u8> = env::read();

    // println!("ckpt_file_bytes len: {}", ckpt_file_bytes.len());
    // println!("tokenizer_file_bytes len: {}", tokenizer_file_bytes.len());
    
    // let ckpt_file = "stories15M.bin"; //&args[1];
    let temperature: f32 = 0.9; // args.get(2).map_or(0.9, |x| x.parse().unwrap());
    let steps: usize = 256; // args.get(3).map_or(256, |x| x.parse().unwrap());
    ...
    let mut rdr = BufReader::new(&ckpt_file_bytes[..]);
    ...
    let (vocab, max_token_length) = read_tokenizer(tokenizer_file_bytes, config.vocab_size as usize);
    ...
    */
    ...
    while pos < steps {
        ...
        // print!("{}", vocab[next].0);
        // append to output string
        output_str.push_str(&vocab[next].0);
        // stdout().flush();
